...
from perlin_noise import get_n_perlin_points
...
def get_gradient_frame(frame_width, frame_height, grid):
    ...
    for ix, vector in enumerate(grid):
        ...
        start_point = (
            i * frame_width // grid_width,
            j * frame_height // grid_height,
        )  # Starting point (scaled grid position)

        end_point = (
            int(
                start_point[0] + scaled_vector[0] * frame_width // grid_width
            ),  # Ending point x
            int(start_point[1] + scaled_vector[1] * frame_height // grid_height),
        )  # Ending point y
...
def get_perlin_frame(frame_width, frame_height, perlin_points):
    ...
    for i, noise_value in enumerate(noise_values):
        ...
        cv.circle(
            frame,
            (int(x * frame_width // grid_width), int(y * frame_height // grid_height)),
            2 * DOT_RADIUS,
            bgr_color,
            -1,
        )
